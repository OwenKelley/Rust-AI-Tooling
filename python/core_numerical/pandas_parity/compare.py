"""Python ↔ Rust Pandas parity + speed comparison harness."""
...
def _default_bin() -> Path:
    ...
    candidates = [
        target_root / "release" / "pandas_parity_runner.exe",
        target_root / "release" / "pandas_parity_runner",
        REPO_ROOT / "target" / "release" / "pandas_parity_runner.exe",
        REPO_ROOT / "target" / "release" / "pandas_parity_runner",
    ]
    for c in candidates:
        if c.exists():
            return c
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to pandas_parity_runner",
    )
    parser.add_argument(
        "--json-out",
        type=Path,
    ...
    if not bin_path.exists():
        ...
        print(
            "build with: cargo build -p parity_runner --bin pandas_parity_runner --release",
            file=sys.stderr,
        )
        return 2
